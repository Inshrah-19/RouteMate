    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-[#f7eaff] from-[62.5%] to-[#948c99]">
      ...
      <main className="flex-1 max-w-[1400px] mx-auto px-4 sm:px-6 md:px-8 py-8 sm:py-10 md:py-12 w-full">
        <div className="mb-8 sm:mb-10">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 md:gap-6 mb-8">
            {stats.map((stat, index) => {
              ...
              return (
                <div key={index} className="bg-white rounded-2xl sm:rounded-3xl p-3 sm:p-4 md:p-6 shadow-lg hover:shadow-2xl transition-all">
                  ...
                  <p className="text-[#6a7282] text-[11px] sm:text-[12px] md:text-[14px] mb-1">{stat.label}</p>
                  <p className="text-[18px] sm:text-[24px] md:text-[32px] font-bold text-[#2c084e]">{stat.value}</p>
                </div>
              )
            })}
          </div>
        ...
        {isRoutesView && (
          <div className="bg-white rounded-2xl sm:rounded-3xl shadow-xl overflow-hidden">
            <div className="bg-gradient-to-r from-[#6412b4] to-[#9333ea] px-4 sm:px-6 md:px-8 py-4 sm:py-6">
              <h2 className="text-[18px] sm:text-[22px] md:text-[24px] font-bold text-white">Active Routes</h2>
              <p className="text-purple-100 text-[12px] sm:text-[13px] md:text-[14px] mt-1">Manage all bus routes in your system</p>
            </div>

            {routes.length === 0 ? (
              <div className="p-6 sm:p-8 text-center text-[#6a7282]">No routes found</div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-purple-50">
                    <tr>
                      <th className="text-left p-3 sm:p-4 md:p-6 text-[#2c084e] font-semibold text-[12px] sm:text-[13px] md:text-[15px]">Route</th>
                      <th className="text-left p-3 sm:p-4 md:p-6 text-[#2c084e] font-semibold text-[12px] sm:text-[13px] md:text-[15px] hidden sm:table-cell">Source</th>
                      <th className="text-left p-3 sm:p-4 md:p-6 text-[#2c084e] font-semibold text-[12px] sm:text-[13px] md:text-[15px] hidden md:table-cell">Destination</th>
                      <th className="text-left p-3 sm:p-4 md:p-6 text-[#2c084e] font-semibold text-[12px] sm:text-[13px] md:text-[15px] hidden md:table-cell">Hours</th>
                      <th className="text-right p-3 sm:p-4 md:p-6 text-[#2c084e] font-semibold text-[12px] sm:text-[13px] md:text-[15px]">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      >
                        <td className="p-3 sm:p-4 md:p-6">
                          <div className="flex items-center gap-2 sm:gap-3">
                            ...
                            <span className="text-[13px] sm:text-[14px] md:text-[16px] font-semibold text-[#2c084e] truncate">{route.routeName}</span>
                          </div>
                        </td>
                        <td className="p-3 sm:p-4 md:p-6 text-[#4a5565] text-[12px] sm:text-[15px] hidden sm:table-cell">{route.source}</td>
                        <td className="p-3 sm:p-4 md:p-6 text-[#4a5565] text-[12px] sm:text-[15px] hidden md:table-cell">{route.destination}</td>
        ...
        {isBusesView && (
          <div className="bg-white rounded-2xl sm:rounded-3xl shadow-xl overflow-hidden">
            <div className="bg-gradient-to-r from-[#6412b4] to-[#9333ea] px-4 sm:px-6 md:px-8 py-4 sm:py-6">
              <h2 className="text-[18px] sm:text-[22px] md:text-[24px] font-bold text-white">Bus Fleet</h2>
              <p className="text-purple-100 text-[12px] sm:text-[13px] md:text-[14px] mt-1">Monitor and manage all buses in your fleet</p>
            </div>

            {buses.length === 0 ? (
              <div className="p-6 sm:p-8 text-center text-[#6a7282]">No buses found</div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-purple-50">
                    <tr>
                      <th className="text-left p-3 sm:p-4 md:p-6 text-[#2c084e] font-semibold text-[12px] sm:text-[13px] md:text-[15px]">Bus</th>
                      <th className="text-left p-3 sm:p-4 md:p-6 text-[#2c084e] font-semibold text-[12px] sm:text-[13px] md:text-[15px]">Type</th>
                      <th className="text-left p-3 sm:p-4 md:p-6 text-[#2c084e] font-semibold text-[12px] sm:text-[13px] md:text-[15px] hidden sm:table-cell">Driver</th>
                      <th className="text-left p-3 sm:p-4 md:p-6 text-[#2c084e] font-semibold text-[12px] sm:text-[13px] md:text-[15px] hidden md:table-cell">Fare</th>
                      <th className="text-left p-3 sm:p-4 md:p-6 text-[#2c084e] font-semibold text-[12px] sm:text-[13px] md:text-[15px] hidden lg:table-cell">Seats</th>
                      <th className="text-right p-3 sm:p-4 md:p-6 text-[#2c084e] font-semibold text-[12px] sm:text-[13px] md:text-[15px]">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      >
                        <td className="p-3 sm:p-4 md:p-6">
                          <div className="flex items-center gap-2 sm:gap-3">
                            ...
                            <span className="text-[13px] sm:text-[14px] md:text-[16px] font-semibold text-[#2c084e] truncate">{bus.busNumber}</span>
                          </div>
                        </td>
                        <td className="p-3 sm:p-4 md:p-6">
                          <span
                        ...
                        <td className="p-3 sm:p-4 md:p-6 text-[#4a5565] text-[12px] md:text-[15px] hidden md:table-cell">${bus.fare}</td>
            ...
            <div className="flex items-center justify-between mb-4 sm:mb-6 pb-4 sm:pb-6 border-b-2 border-gray-100">
              <div className="flex items-center gap-2 sm:gap-4">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[12px] sm:text-[14px] text-[#6a7282]">Fill in the details</p>
                </div>
              </div>
              <button
                onClick={() => setShowAddModal(false)}
