...
// Utility function to generate dynamic schedules
const generateSchedule = (departureTime: string, arrivalTime: string) => {
  ...
  const formatTime = (mins: number) => {
    ...
    return `${String(hours).padStart(2, "0")}:${String(minsRemainder).padStart(
      2,
      "0"
    )} ${period}`;
  };
      ...
      <main className="flex-1 max-w-[1400px] mx-auto px-4 sm:px-6 md:px-8 py-6 sm:py-12 w-full">
        <div className="mb-8 sm:mb-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-[14px] sm:text-[16px] text-[#6a7282]">
              View schedules and track all available routes
            </p>
          </div>
          <div className="bg-white rounded-[14px] sm:rounded-2xl p-3 sm:p-4 md:p-6 shadow-lg border-2 border-purple-100 flex-shrink-0">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div>
                <p className="text-[11px] sm:text-[13px] md:text-[14px] text-[#6a7282]">
                  Last Updated
                </p>
                <p className="text-[13px] sm:text-[15px] md:text-[16px] font-bold text-[#2c084e]">
                  Just now
                </p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 mb-8 sm:mb-10">
          <div className="bg-gradient-to-br from-purple-500 to-pink-500 rounded-[16px] sm:rounded-3xl p-4 sm:p-6 shadow-xl text-white">
            ...
            <p className="text-[13px] sm:text-[14px] md:text-[15px] opacity-90">
              Total Buses
            </p>
          </div>
        ...
        {buses.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-[#6a7282] text-[16px] sm:text-[18px]">
          ...
          <div className="space-y-4 sm:space-y-6">
            {buses.map((bus, index) => {
              const timetableData = generateSchedule(
                bus.departureTime,
                bus.arrivalTime
              );
                ...
                >
                  <div
                    className={`bg-gradient-to-r ${getTabGradient(
                      selectedTab
                    )} p-3 sm:p-4 md:p-6 cursor-pointer sm:cursor-default select-none`}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 text-white/90 text-[13px] sm:text-[14px] md:text-[16px] truncate">
                            ...
                            <p className="truncate">
                              {bus.Route?.routeName || "Route info"}
                            </p>
                          </div>
                        </div>
                      </div>
                        ...
                        {timetableData.map((time, idx) => (
                          ...
                          >
                            <div className="flex items-start justify-between gap-2 mb-2 sm:mb-3">
                              <div>
                                <p className="text-[11px] sm:text-[12px] text-[#6a7282] mb-0.5 sm:mb-1">
                                  Departure
                                </p>
