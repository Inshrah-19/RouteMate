...
const generateSchedule = (departureTime: string, arrivalTime: string) => {
  ...
  const formatTime = (mins: number) => {
    ...
    return `${String(hours).padStart(2, "0")}:${String(minsRemainder).padStart(2, "0")} ${period}`
  }

  const startMinutes = parseTime(departureTime)
  const endMinutes = parseTime(arrivalTime)

  let journeyDuration = endMinutes - startMinutes
  if (journeyDuration < 0) journeyDuration += 24 * 60

  const schedule = []
  const MAX_SCHEDULES = 10

  // Calculate interval to fit max 10 schedules in a day
  let interval = 15 // default 15 minutes
  if (journeyDuration > 0) {
    // Estimate: generate schedules from actual departure time throughout the day
    const dayDuration = 24 * 60 // 1440 minutes
    interval = Math.max(15, Math.floor(dayDuration / MAX_SCHEDULES))
  }

  // Generate schedules starting from the bus's departure time
  for (let i = 0; i < MAX_SCHEDULES; i++) {
    const currentTime = (startMinutes + i * interval) % (24 * 60)
    const arrival = (currentTime + journeyDuration) % (24 * 60)

    schedule.push({
      departure: formatTime(currentTime),
      arrival: formatTime(arrival),
      duration: `${Math.floor(journeyDuration / 60)}h ${journeyDuration % 60}m`,
      status: "On-time",
...
export default function BusTimetables() {
  ...
  const [allBuses, setAllBuses] = useState<any[]>([]) // Keep all buses for stats
  ...
  const filterBuses = () => {
    filterBusesByTab(allBuses, selectedTab)
  }

  const filterBusesByTab = (data: any[], tab: string) => {
    if (tab === "all") {
      setBuses(data)
    } else {
      setBuses(data.filter((b) => b.busType === tab))
    }
  }
      ...
      <main className="flex-1 max-w-[1400px] mx-auto px-4 sm:px-6 md:px-8 py-6 sm:py-12 w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 mb-8 sm:mb-10">
          <div className="bg-gradient-to-br from-purple-500 to-pink-500 rounded-[16px] sm:rounded-3xl p-4 sm:p-6 shadow-xl text-white">
            ...
            <p className="text-[24px] sm:text-[28px] md:text-[32px] font-extrabold mb-1">{allBuses.length}</p>
          ...
          <div className="bg-gradient-to-br from-blue-500 to-cyan-500 rounded-[16px] sm:rounded-3xl p-4 sm:p-6 shadow-xl text-white">
            ...
            <p className="text-[24px] sm:text-[28px] md:text-[32px] font-extrabold mb-1">
              {allBuses.filter((b) => b.busType === "AC").length}
            </p>
          ...
          <div className="bg-gradient-to-br from-green-500 to-emerald-500 rounded-[16px] sm:rounded-3xl p-4 sm:p-6 shadow-xl text-white">
            ...
            <p className="text-[24px] sm:text-[28px] md:text-[32px] font-extrabold mb-1">
              {allBuses.filter((b) => b.busType === "Green").length}
            </p>
          ...
          <div className="bg-gradient-to-br from-orange-500 to-red-500 rounded-[16px] sm:rounded-3xl p-4 sm:p-6 shadow-xl text-white">
            <Bus className="w-8 h-8 sm:w-10 mb-2 sm:mb-3 opacity-80" />
            <p className="text-[24px] sm:text-[28px] md:text-[32px] font-extrabold mb-1">
              {allBuses.filter((b) => b.busType === "Non-AC").length}
            </p>
        ...
        {buses.length === 0 ? (
          ...
          <div className="space-y-4 sm:space-y-6">
            {buses.map((bus, index) => {
              const timetableData = generateSchedule(bus.departureTime, bus.arrivalTime)
